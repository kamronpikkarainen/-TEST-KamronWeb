 ...
 * that sweeps the card obeys the page's key light — it always enters
 * from the upper-left (see src/lib/lighting.js).
 */
...
function TiltCard({ children, className = '' }) {
    ...
    const onMove = (e) => {
      ...
      if (sheen) {
        ...
        sheen.style.background = `radial-gradient(circle at ${(px + 0.5) * 100}% ${
          (py + 0.5) * 100
        }%, rgba(255,217,168,0.14), transparent 55%)`;
      }
    };
    const onLeave = () => {
      xTo(0);
  ...
  return (
    <div ref={wrap} className={className} style={{ perspective: '1100px' }}>
      <div
        ref={inner}
        className="key-sheen relative h-full overflow-hidden rounded-2xl border border-white/10 bg-panel"
        ...
        <div className="pf-grid mt-14 grid gap-6 lg:grid-cols-2">
          ...
          <TiltCard className="pf-card lg:col-span-2">
            <div className="grid gap-0 md:grid-cols-2">
              ...
              <div className="relative min-h-[260px] overflow-hidden bg-gradient-to-b from-[#0d2416] to-[#081a10] p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-base font-extrabold text-[#eaf5e2]">GreenEdge</div>
                ...
                <div className="mt-6 text-2xl font-extrabold leading-tight text-[#f2f8ee]">
                  A sharper lawn,
                  <br />
                  handled.
                </div>
                <div className="mt-4 inline-block rounded-full bg-accent px-4 py-1.5 text-[10px] font-bold text-[#0A1A0E]">
                  Book my yard
                </div>
                <div
                  aria-hidden="true"
                  className="absolute -bottom-6 -right-6 h-36 w-44 rounded-xl opacity-80"
                  style={{
                    background:
              ...
              <div className="flex flex-col justify-between p-7 sm:p-9">
                <div>
                  ...
                  <p className="mt-4 rounded-lg border border-dashed border-white/15 px-3 py-2 text-xs text-mute/70">
                    {featured.metrics}
                  </p>
                </div>
                <div className="mt-6 flex items-center gap-4">
                  <div className="flex h-16 w-24 items-center justify-center rounded-md border border-dashed border-white/15 text-center text-[9px] leading-tight text-mute/60">
                    [PLACEHOLDER —<br />
                    before screenshot]
                  </div>
              ...
              <div className="flex h-full min-h-[240px] flex-col justify-between p-7 sm:p-9">
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="rounded-full border border-white/15 bg-white/5 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-mute">
                      Placeholder
                    </span>
                  </div>
                  <p className="mt-1 text-xs text-mute">{p.tag}</p>
                </div>
                <div className="mt-8 flex h-28 items-center justify-center rounded-lg border border-dashed border-white/15 text-xs text-mute/60">
